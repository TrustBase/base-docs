...
trustbase will support the `contract-pallet` for setting up and managing WebAssembly smart contracts written with ink!.

This article will walk through the compiling and deployment process for an Ink smart contract, before visiting the means of calling contract functions through the Polkadot JS client.
...
The `Erc20` contract is a simple contract.
